...
class PermissionTester: TestRunner {
    ...
    private func testFirstLaunchDetection() -> TestResult {
        ...
        let originalVersion = UserDefaults.standard.string(forKey: "manicrypt_last_version")
        let originalWelcome = UserDefaults.standard.bool(forKey: "manicrypt_has_shown_welcome")
        
        // Test 1: Première installation (aucune version)
        UserDefaults.standard.removeObject(forKey: "manicrypt_last_version")
        UserDefaults.standard.removeObject(forKey: "manicrypt_has_shown_welcome")
        UserDefaults.standard.synchronize()
        
        let currentVersion = Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? "1.0"
        let lastVersion = UserDefaults.standard.string(forKey: "manicrypt_last_version")
        let hasShownWelcome = UserDefaults.standard.bool(forKey: "manicrypt_has_shown_welcome")
        ...
        UserDefaults.standard.set("0.9", forKey: "manicrypt_last_version")
        UserDefaults.standard.synchronize()
        
        let oldVersion = UserDefaults.standard.string(forKey: "manicrypt_last_version")
        ...
        if let original = originalVersion {
            UserDefaults.standard.set(original, forKey: "manicrypt_last_version")
        } else {
            UserDefaults.standard.removeObject(forKey: "manicrypt_last_version")
        }
        UserDefaults.standard.set(originalWelcome, forKey: "manicrypt_has_shown_welcome")
        UserDefaults.standard.synchronize()
    ...
    private func testVersionManagement() -> TestResult {
        ...
        UserDefaults.standard.set(currentVersion, forKey: "manicrypt_last_version")
        UserDefaults.standard.synchronize()
        
        let savedVersion = UserDefaults.standard.string(forKey: "manicrypt_last_version")
        tests.append(savedVersion == currentVersion ? "✅ Sauvegarde version OK" : "❌ Sauvegarde version échouée")
        
        // Test 3: Détection de changement
        UserDefaults.standard.set("0.8", forKey: "manicrypt_last_version")
        UserDefaults.standard.synchronize()
    ...
    private func testPreferencesPersistence() -> TestResult {
        ...
        let testKey = "manicrypt_test_preference"
        ...
        // Test 4: Préférences Manicrypt existantes
        let manicryptKeys = getAllManicryptPreferences()
        tests.append("📊 Préférences Manicrypt: \(manicryptKeys.count) clés")
    ...
    private func getAllManicryptPreferences() -> [String: Any] {
        ...
        return defaults.filter { key, _ in
            key.lowercased().contains("manicrypt") ||
            key == "useGlobalHotkeys" ||
            key.hasPrefix("manicrypt_")
        }
    }
}
