...
class KeychainTester: TestRunner {
    ...
    func runTests() -> [TestResult] {
        ...
        // Test 5: Manicrypt spécifique
        results.append(testManicryptKeychain())
    ...
    /// Test spécifique pour Manicrypt
    func testManicryptKeychain() -> TestResult {
        print("5️⃣ Test spécifique du Keychain Manicrypt...")
        
        do {
            // Test de stockage
            try SecureKeychainManager.shared.storeGlobalPassphrase("test_passphrase_manicrypt")
            ...
            return TestResult(.passed, "Cycle complet Keychain Manicrypt")
            
        } catch {
            return TestResult(.failed, "Erreur test Manicrypt: \(error)")
        }
    }
